...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/28 13:18:01 by panne-ro          #+#    #+#             */
/*   Updated: 2026/04/28 13:18:02 by panne-ro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (_HitPoints <= 0)
	{
		std::cout << MAGENTA << "ClapTrap " << _name << " can't take damage cause he is dead" << std::endl;
		return ;
	}
	std::cout << RED << "ClapTrap " << _name << " take " << amount << " damages !" << std::endl;
	_HitPoints -= amount;
	isAlive();
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	_EnergyPoints--;
	_HitPoints += amount;
	std::cout << GREEN << "ClapTrap " << _name << " repairs itself of " << amount << " health points !" << std::endl;
}
...
int	ClapTrap::isAlive()
{
	if (_HitPoints <= 0)
	{
		std::cout << MAGENTA << "ClapTrap " << _name << " is dead" << std::endl;
		return (0);
	}
	else
		return (1);
...
int	ClapTrap::haveEnergy()
{
	if (_EnergyPoints <= 0)
	{
		std::cout << YELLOW << "ClapTrap " << _name << " have not enough energy" << std::endl;
		return (0);
	}
	else
		return (1);
